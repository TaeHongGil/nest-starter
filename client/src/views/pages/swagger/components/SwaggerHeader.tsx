...
const SwaggerHeader = observer(() => {
  document.title = `${ServerConfig.server_name.toUpperCase()} - ${ServerConfig.zone.toUpperCase()} ${protocolStore.server_type.toUpperCase()} DOCUMENT ${SwaggerMetadata.config.version}`;
  ...
  return (
    <AppBar position="static" style={{ backgroundColor: '#3c3c3c', boxShadow: 'none' }}>
      <Toolbar variant="dense">
        ...
        <Button size="small" variant="contained" color="warning" onClick={async () => protocolStore.toggleServerType()} disableRipple sx={{ marginRight: 1 }}>
          Change Swagger Mode - {protocolStore.server_type.toUpperCase()}
        </Button>
