...
export function ColorPreview({ sx, colors, className, slotProps, gap = 6, limit = 3, size = 16, ...other }: ColorPreviewProps) {
  ...
  return (
    <ColorPreviewRoot className={mergeClasses([colorPreviewClasses.root, className])} sx={sx} {...other}>
      ...
      {colors.length > limit && <ItemLabel className={colorPreviewClasses.label} {...slotProps?.label}>{`+${remainingColorCount}`}</ItemLabel>}
    </ColorPreviewRoot>
  );
}
